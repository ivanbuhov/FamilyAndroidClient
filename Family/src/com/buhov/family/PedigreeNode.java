...
public class PedigreeNode {
	...
	public class PersonNode {
		...
		public boolean hasAnyParents() {
			return (this.hasFirstParent() || this.hasSecondParent());
		}
		...
		public ArrayList<PersonNode> getSiblings() {
			PersonNode parent = this.getFirstParent();
			if(parent == null) {
				parent = this.getSecondParent();
				if(parent == null) {
					return new ArrayList<PersonNode>();
				}
			}
			
			ArrayList<PersonNode> siblings = parent.getChildren();
			for(int i = 0; i < siblings.size(); i++) {
				if(siblings.get(i).getData().getId() == this.getData().getId()) {
					siblings.remove(i);
					break;
				}
			}
		...
		public int getParentsCount() {
			int count = 0;
			if(this.hasFirstParent()) {
				count++;
			}
			if(this.hasSecondParent()) {
				count++;
			}
			return count;
		}
